...
class Resize3D(Module):
    ...
    @staticmethod
    def suggest_optimal_size(size, factor=1.0):
        """
        Suggests a size where all dimensions are divisible by 8, considering a scaling factor.
        ...
        Returns:
            tuple: Suggested size (D', H', W') with all dimensions divisible by 8.
        """
        if factor <= 0:
            raise ValueError("Factor must be greater than zero.")
        
        def nearest_multiple_of_8(dim):
            lower = (dim // 8) * 8
            upper = lower + 8
            return lower if abs(lower - dim) < abs(upper - dim) else upper

        # Scale the size using the factor, ensure integer values, then round to nearest multiple of 8
        scaled_size = tuple(int(round(dim / factor)) for dim in size)
        optimal_size = tuple(nearest_multiple_of_8(dim) for dim in scaled_size)
